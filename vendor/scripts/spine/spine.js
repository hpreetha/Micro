(function() {
  ...
  Model = (function(_super) {
    ...
    Model.configure = function() {
 	...
 	Model.configureWithType = function() {
      var attributesWithType,attributes = [], name, _i;
      name = arguments[0], attributesWithType = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
      this.className = name;
      this.fields = attributesWithType;
      this.records = {};
      this.crecords = {};
      if(attributesWithType.length){
      	for(_i = 0; _i< attributesWithType.length; _i++){
      		if(Object.prototype.toString.call(attributesWithType[_i]).slice(8, -1) == "Object"){
      			attributes.push(attributesWithType[_i].name)	
      		}
      		else if(Object.prototype.toString.call(attributesWithType[_i]).slice(8, -1) == "String"){
      			attributes.push(attributesWithType[_i])	
      		}
      	}
      }
      if (attributes.length) this.attributes = attributes;
      ...
      return this;
   	...
   	Model.getAttributeType = function(attribute) {
		var attributeType, _i;
		if(attribute) {
			for( _i = 0; _i < this.fields.length; _i++) {
				if(Object.prototype.toString.call(this.fields[_i]).slice(8, -1) == "Object") {
					if(this.fields[_i].name == attribute) {
						attributeType = this.fields[_i].type;
						break;
					}
    ...
    Model.prototype.isDirty = function() {
		var savedRecord, thisRecord;
		thisRecord = this.attributes();
		savedRecord = this.constructor.findByAttribute('id', thisRecord.id);
		if(savedRecord) {
			tuiForms.savedRecord = savedRecord.attributes();
			if(JSON.stringify(thisRecord) == JSON.stringify(savedRecord.attributes())) {
				return false;
			} else {
				return true;
			}
